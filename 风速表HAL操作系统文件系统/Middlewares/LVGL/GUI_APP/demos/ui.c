...
float fengsu_second=0;

int shangchuan_flag=0;
...
void my_timer_show_fengsu(lv_timer_t *timer)
{
	...
	last_state=fengsu_state;
	
	if(ms==200){
		ms=0;
    lv_label_set_text_fmt(ui_Screen1_Label_countdown, "%ds", 10 - count_second);
    count_second++;
    ...
    //风速
		if(Total_times==0){
			...
			fengsu_second=(float)Total_times/4*0.0594+0.1478;
			total_numbers+=Total_times;
			Total_times=0;
		}
    ...
    if (count_second == 11)
    {
				fengsu_second=(float)total_numbers/40*0.0594+0.1478;
			  lv_label_set_text_fmt(ui_Screen1_Label_wind_speed, "%.2f",fengsu_second);
...
void ui_event_record_start_bg(lv_event_t *e)
{
    ...
    if (event_code == LV_EVENT_CLICKED)
    {
        ...
        {
            flag_start_timer = 1;
            timer_show_fengsu = lv_timer_create(my_timer_show_fengsu, 5, NULL);
        }
        //(e);
    }
}
void ui_event_record_end_bg(lv_event_t *e)
{
    ...
    if (event_code == LV_EVENT_CLICKED)
    {
        if (flag_start_timer == 0)
        {
            ...
            lv_fs_file_t f;
            lv_fs_res_t res;
            sprintf((char *)write_buffer," %04d.%02d.%02d %02d:%02d:%02d,%3d,%.2fm/s\n", year,month,day,hour, minute, second,cedian_number,fengsu_second);
            res = lv_fs_open(&f, "0:/fengsu_data1.txt", LV_FS_MODE_WR);
            lv_fs_seek(&f, 0, LV_FS_SEEK_END);
            lv_fs_write(&f, (unsigned char *)write_buffer, 32, NULL);
            lv_fs_close(&f);
						
						if(shangchuan_flag==1){
							unsigned char send_buffer[50] = {0};
							sprintf((char *)send_buffer,"{\"fengsu\":%.2f}",fengsu_second);
...
//     (e);
        ...
        lv_fs_file_t f;
        lv_fs_res_t res;
        res = lv_fs_open(&f, "0:/fengsu_data1.txt", LV_FS_MODE_RD);
        lv_fs_seek(&f, page_data*32, LV_FS_SEEK_CUR);
        lv_fs_read(&f, (unsigned char *)write_buffer, 32, NULL);
        lv_fs_close(&f);
				lv_label_set_text_fmt(ui_neirong_bg2, "%s", write_buffer);
...
void ui_Screen_menu1_screen_init(void)
{
    ...
    lv_label_set_text(ui_menu2_name1, "查看");
    ...
    lv_label_set_text(ui_time1, "2023.05.15 12:00:00");
    ...
    lv_label_set_text(ui_Screen1_Label_countdown, "10s");
    ...
    lv_label_set_text(ui_Screen1_Label_ms, "m/s");
    ...
    lv_label_set_text(ui_last_bg2, "上页");
       lv_obj_set_style_text_font(ui_last_bg2, &ui_font_Langmanyayuan16, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_label_set_text(ui_Label2, "下页");
       lv_obj_set_style_text_font(ui_Label2, &ui_font_Langmanyayuan16, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_label_set_text(ui_shangchuan_bg3, "上传");
       lv_obj_set_style_text_font(ui_shangchuan_bg3, &ui_font_Langmanyayuan16, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_label_set_text(ui_duishi_bg3, "对时");
       lv_obj_set_style_text_font(ui_duishi_bg3, &ui_font_Langmanyayuan16, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_add_event_cb(ui_cedian_ge_bg, ui_event_cedian_ge_bg, LV_EVENT_ALL, NULL);
		lv_obj_add_event_cb(ui_last_page_bg2, ui_event_last_page_bg2, LV_EVENT_ALL, NULL);
		lv_obj_add_event_cb(ui_next_page_bg2, ui_event_next_page_bg2, LV_EVENT_ALL, NULL);
		lv_obj_add_event_cb(ui_Switch_shangchuan_bg3, ui_event_Switch_shangchuan_bg3, LV_EVENT_ALL, NULL);
		lv_obj_add_event_cb(ui_Switch_duishi_bg3, ui_event_Switch_duishi_bg3, LV_EVENT_ALL, NULL);
    ...
    lv_group_add_obj(g, ui_record_end_bg);
		lv_group_add_obj(g, ui_last_page_bg2);
		lv_group_add_obj(g, ui_next_page_bg2);
		lv_group_add_obj(g, ui_Switch_shangchuan_bg3);
		lv_group_add_obj(g, ui_Switch_duishi_bg3);
