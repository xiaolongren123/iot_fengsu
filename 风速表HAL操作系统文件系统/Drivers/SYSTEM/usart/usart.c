...
uint8_t SerialData[36];      //Com数据接收缓存
uint8_t SerialDataLen=0;  //Com数据接收长度
uint8_t SerialDataSta=0;  //Com数据接收状态
...
//    {
//        if (g_usart_rx_sta==0)             /* 接收未完成 */
//        {
//            if (g_rx_buffer[0] != '"')             /* 接收到的不是0x0a（即不是换行键） */
//            {
//                g_usart_rx_sta = 0;                 /* 接收错误,重新开始 */
//            }
//            else                                    /* 接收到的是0x0a（即换行键） */
//            {
//                g_usart_rx_sta=1;           /* 接收完成了 */
//            }
...
//    }
		...
		if (huart->Instance == USART_UX)                    /* 如果是串口1 */
    {
					...
					if (g_rx_buffer[0] == ':'||num_uart>0)             /* 接收到的不是0x0a（即不是换行键） */
            {
							  g_usart_rx_buf[num_uart] = g_rx_buffer[0];
								num_uart++;
            }
